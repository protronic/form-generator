...
let ALERT_BUTTON_TEXT="ok";function createCustomAlert(e,t){if(null==t&&(t="Fehler"),document.getElementById("modalContainer"))return;let n=document.body.appendChild(document.createElement("div"));n.id="modalContainer",n.style.height=document.documentElement.scrollHeight+"px";let o=n.appendChild(document.createElement("div"));o.id="alertBox",o.classList.add(`${t.toLowerCase()}-alert`),document.all&&!window.opera&&(o.style.top=document.documentElement.scrollTop+"px"),o.style.left=(document.documentElement.scrollWidth-o.offsetWidth)/2+"px",o.appendChild(document.createElement("h1")).appendChild(document.createTextNode(t)),o.appendChild(document.createElement("p")).innerHTML=e;let d=o.appendChild(document.createElement("a"));d.id="closeBtn",d.appendChild(document.createTextNode(ALERT_BUTTON_TEXT)),d.href="#",d.onclick=function(){return removeCustomAlert(),!1},document.addEventListener("keydown",enterPressedListener)}function enterPressedListener(e){if(13==e.which)return document.removeEventListener("keydown",enterPressedListener),removeCustomAlert(),!1}function removeCustomAlert(){document.getElementsByTagName("body")[0].removeChild(document.getElementById("modalContainer"))}module.exports={createCustomAlert:createCustomAlert};

},{}],2:[function(require,module,exports){
const{InputFieldText:InputFieldText}=require("./input-field-generic.js"),DependenceMixin=module.exports.DependenceMixin=(e=>(class extends e{constructor(){super(),this.defaultOptions={...this.defaultOptions,abhaengigFeld:"",wertSichtbar:"",interval:500},this.visibility=!0,this.hideField()}applyTemplate(){super.applyTemplate(),this.linkDependency()}checkDependence(e,i){e.getModel()===this.options.wertSichtbar?this.showField():this.hideField()}linkDependency(){let e=this.parentElement.querySelector(`*[name='${this.saveValue("abhaengigFeld",this.options.abhaengigFeld)}']`);e.addEventListener("form-valid",this.checkDependence.bind(this,e)),setInterval(this.checkDependence.bind(this,e),this.options.interval)}hideField(){this.visibility&&(this.visibility=!1,this.classList.add("hidden"))}showField(){this.visibility||(this.visibility=!0,this.classList.remove("hidden"),this.checkValidity())}getModel(){return this.visibility?super.getModel():void 0}checkValidity(){return!this.visibility||super.checkValidity()}}));module.exports.InputFieldAbhaengig=class extends(DependenceMixin(InputFieldText)){constructor(){super()}};

},{"./input-field-generic.js":10}],3:[function(require,module,exports){
require("./history-input-extender.js");const{InputFieldObject:InputFieldObject}=require("./input-field-object.js"),{fieldTypeMap:fieldTypeMap}=require("./formular-components.js"),{createCustomAlert:createCustomAlert}=require("./custom-alert-box.js"),{sendLogToLogstash:sendLogToLogstash}=require("./logging-connector.js");var baseUrl="http://10.19.28.94:8087",schemaPath="/schema",modelPath="/model";function sendErrorReport(e,t,n,o){let r=Object.assign({appname:"json-form-creator",audit:!0,form:t,model:n,error:e,message:o});throw sendLogToLogstash(r),new Error(e)}Object.keys(fieldTypeMap).forEach(e=>{customElements.define(fieldTypeMap[e].tag,fieldTypeMap[e].conName)});class SearchParams{constructor(e){let t=this;e.slice(1).split("&").forEach(function(e){let n=e.split("=")[0],o=e.split("=")[1];t[n]=o})}append(e,t){this[e]=t}get(e){return this[e]}set(e,t){this[e]=t}delete(e){delete this[e]}toString(){return"?"+Object.keys(this).map(e=>`${e}=${this[e]}`).join("&")}}function prepareModel(e,t){let n="",o=(new Date).getTime();try{n=wikiContext.UserName}catch(e){console.log("wikiContext not in scope.")}return e["#parentForm"]=t,e["#changed_user"]=n,e["#changed_time"]=o,JSON.stringify(e)}function transformToHistoryModel(e){let t={};return function e(t,n){t&&Object.keys(t).forEach(o=>{"object"==typeof t[o]?e(t[o],n):n[o]=t[o]})}(e,t),t}function fetchGlobalHistoryModels(e){fetch(`${baseUrl}${modelPath}?parentForm=${e}`).then(e=>e.json()).then(e=>e.map(e=>e.log)).then(e=>e.map(e=>{try{return JSON.parse(e)}catch(e){}})).then(e=>e.map(e=>transformToHistoryModel(e))).then(e=>{console.log(e),window.postMessage(JSON.stringify({messageType:"history-source-models",messageData:e}))})}function uploadNewModel(e,t){let n=prepareModel(e,t);return fetch(`${baseUrl}${modelPath}`,{method:"POST",body:`${n}`,headers:{"Content-Type":"application/json"}}).then(n=>{if(n.ok)return n.json();sendErrorReport(`Verbindung zur Datenbank ist Fehlgeschlagen mit status ${n.status} und folgender Meldung: ${n.statusText}.`,JSON.stringify(t),JSON.stringify(e),`URL: ${baseUrl}${modelPath}}`)}).then(e=>(console.log(e),e._id))}function uploadExistingModel(e,t){let n=prepareModel(e,t);return fetch(`${baseUrl}${modelPath}/${e["#modelID"]}`,{method:"POST",body:n,headers:{"Content-Type":"application/json"}}).then(n=>{if(n.ok)return n.json();sendErrorReport(`Verbindung zur Datenbank ist Fehlgeschlagen mit status ${n.status} und folgender Meldung: ${n.statusText}.`,JSON.stringify(t),JSON.stringify(e),`URL: ${baseUrl}${modelPath}/${e["#modelID"]}`)}).then(n=>{0!==Object.keys(n).length&&sendErrorReport("Speichern des Models ist aus einem unbekannten Grund fehlgeschlagen. Für mehr Informationen mit 'r.seidler' in Verbindung setzen.",JSON.stringify(t),JSON.stringify(e),`URL: ${baseUrl}${modelPath}/${e["#modelID"]}`),console.log(n)})}function removeExistingModel(e,t){let n=e["#modelID"];return fetch(`${baseUrl}${modelPath}/${n}`,{method:"DELETE"}).then(o=>{o.ok||sendErrorReport(`Beim Löschen des models hat die Datenbank einen Fehlercode zurückgegeben. Fehlercode: ${o.status}; Fehlernachricht: ${o.statusText}`,JSON.stringify(t),JSON.stringify(e),`URL: ${baseUrl}${modelPath}/${n}`)})}function loadModelFromDB(e,t){return fetch(`${baseUrl}${modelPath}/${e}`).then(n=>{if(n.ok)return n.json();sendErrorReport(`Beim Laden des models hat die Datenbank einen Fehlercode zurückgegeben. Fehlercode: ${n.status}; Fehlernachricht: ${n.statusText}`,JSON.stringify(t),"",`URL: ${baseUrl}${modelPath}/${e}`)}).then(t=>(console.log({model:t}),t["#modelID"]=e,t))}function loadSchemaFromDB(e){if(null!=e)return fetch(`${baseUrl}${schemaPath}/${e}`).then(t=>{if(t.ok||500==t.status)return t.json();sendErrorReport(`Datenbank Anfrage gab Fehlercode ${t.status}, mit folgender Nachricht zurück: "${t.statusText}".`,"","",`URL: ${baseUrl}${schemaPath}/${e}`)}).then(t=>{if(null==t.error)return t;sendErrorReport(`Schema #${e} konnte in der Datenbank nicht gefunden werden.`,"","",`URL: ${baseUrl}${schemaPath}/${e}`)});createCustomAlert("Es ist kein Schema angegeben. In der URL muss der Parameter 'schema' mit einer gültigen schema ID angegeben sein (zB.: http://wiki.protronic-gmbh.de:8087/form-gen/prot-form-gen.html?schema=1012).")}function getSchemaId(){return new SearchParams(location.search).get("schema")}class FormCreator extends InputFieldObject{constructor(){super(),this.model={}}connectedCallback(){this.rootElement=document.createElement("form"),this.rootElement.classList.add("form-root"),this.appendChild(this.rootElement),baseUrl=this.getAttribute("data-url")?this.getAttribute("data-url"):baseUrl,loadSchemaFromDB(getSchemaId()).then(e=>{this.applySchema(e)}).catch(e=>{createCustomAlert(e.message)}),fetchGlobalHistoryModels()}applySchema(e){console.log({schema:e}),this.schema=e,this.options.name=e.formular,this.options.label=`${e.formular}`,this.options.subform=e.felder;try{let t=this.modelId=new SearchParams(location.search).get("mid");t?loadModelFromDB(t,e.formular).then(n=>{this.model=this.convertValue("initialModel",JSON.stringify(n)),this.model["#modelID"]=t,this.options.name=e.formular,this.options.label=`${this.options.label} ${t}`,this.options.initialModel=this.convertValue("initialModel",JSON.stringify(n)),this.rootElement.options=this.options,this.applyTemplate()}).catch(e=>createCustomAlert(e.message,"Fehler")):(this.options.initialModel=this.loadFromLocal(),this.rootElement.options=this.options,this.applyTemplate())}catch(e){console.log(this.options.initialModel),console.error(e)}this.createButtonContainer(),this.createExplanation(),this.renderGeneralInfo(e)}renderGeneralInfo(e){this.insertAdjacentHTML("afterbegin",`\n            <div class="formular-title">\n                <h2>${e.formular}</h2>\n            </div>\n        `)}testModelCreation(){let e=document.createElement("button");e.setAttribute("type","button"),e.addEventListener("click",e=>{this.checkValidity()?(this.model={...this.model,...this.getModel()},this.saveFormLocal(this.model["#modelID"],this.model)):console.error("Einige Formular Elemente sind nicht korrekt ausgefüllt.")}),e.innerText="Erzeuge Model",this.appendChild(e)}createButtonContainer(){this.buttonContainer=document.createElement("div"),this.buttonContainer.classList.add("btn-container"),this.appendChild(this.buttonContainer),this.uploadModelButton(),this.newFormularButton(),this.removeButton(),this.copyModelButton()}createNewFormURL(e){let t=new URL(location.href),n=new SearchParams(location.search);return n.set("lsid","null"),n.delete("mid"),void 0!==e?(n.set("mid",e),t.search=n.toString(),history.pushState({schema:n.get("schema"),mid:e},"",t.toString())):(t.search=n.toString(),history.pushState({schema:n.get("schema")},"",t.toString())),t.href}newFormularButton(){let e=document.createElement("button");e.setAttribute("type","button"),e.addEventListener("click",this.newFormularButtonClickListener.bind(this)),document.addEventListener("keydown",this.newFormularButtonHotkeyListener.bind(this)),e.innerText="Neu Anlegen",this.buttonContainer.appendChild(e)}newFormularButtonHotkeyListener(e){e.ctrlKey&&45==e.which&&(this.newFormularButtonClickListener.bind(this,void 0)(),e.preventDefault())}newFormularButtonClickListener(e){this.createNewFormURL(),this.remove(),document.body.append(document.createElement("prot-form-gen"))}removeButton(){let e=document.createElement("button");e.setAttribute("type","button"),e.addEventListener("click",this.removeButtonClickListener.bind(this)),document.addEventListener("keydown",this.removeButtonHotkeyListener.bind(this)),e.innerText="Löschen",this.buttonContainer.appendChild(e)}removeButtonHotkeyListener(e){e.ctrlKey&&46==e.which&&(this.removeButtonClickListener.bind(this,void 0)(),e.preventDefault())}removeButtonClickListener(e){this.model["#modelID"]&&confirm(`Are you sure, you want to renove model with id: ${this.model["#modelID"]}?`)&&removeExistingModel(this.model,this.schema.formular).then(()=>{this.createNewFormURL(),this.remove(),document.body.append(document.createElement("prot-form-gen"))}).catch(e=>createCustomAlert(e.message,"Fehler"))}uploadModelButton(){let e=document.createElement("button");e.setAttribute("type","button"),e.addEventListener("click",this.uploadModelButtonClickListener.bind(this)),document.addEventListener("keydown",this.uploadModelButtonHotkeyListener.bind(this)),e.innerText="Speichern",this.buttonContainer.appendChild(e)}uploadModelButtonHotkeyListener(e){e.ctrlKey&&83==e.which&&(this.uploadModelButtonClickListener.bind(this,void 0)(),e.preventDefault())}uploadModelButtonClickListener(e){this.checkValidity()?(this.model={...this.getModel(),"#modelID":this.model["#modelID"]},void 0===this.model?console.log("empty model"):(window.postMessage(JSON.stringify({messageType:"submit-msg",messageData:transformToHistoryModel(this.model)})),this.saveFormLocal(this.model["#modelID"],this.model),this.model["#modelID"]?(console.log(this.model),uploadExistingModel(this.model,this.schema.formular).then(()=>createCustomAlert(`Änderungen an ${this.model["#modelID"]} wurden gespeichert.`,"Erfolg")).catch(e=>createCustomAlert(e.message,"Fehler"))):uploadNewModel(this.model,this.schema.formular).then(e=>{this.model["#modelID"]=e,this.createNewFormURL(e),this.querySelector(`label[for="${this.schema.formular}"]`).innerText=`${this.options.label} ${e}`,createCustomAlert(`Daten wurden erfolgreich in der Datenbank unter folgender ID abgelegt.\n${e}`,"Erfolg")}).catch(e=>createCustomAlert(e.message,"Fehler")))):console.log("invalid")}copyModelButton(){let e=document.createElement("button");e.setAttribute("type","button"),e.addEventListener("click",this.copyModelButtonClickListener.bind(this)),document.addEventListener("keydown",this.copyModelButtonHotkeyListener.bind(this)),e.innerText="Kopieren",this.buttonContainer.appendChild(e)}copyModelButtonHotkeyListener(e){e.ctrlKey&&75==e.which&&(this.copyModelButtonClickListener.bind(this,void 0)(),e.preventDefault())}copyModelButtonClickListener(e){this.model=this.getModel(),this.remove(),this.model["#modelID"]=void 0,uploadNewModel(this.model,this.schema.formular).then(e=>{this.model["#modelID"]=e,location.href=this.createNewFormURL(e)}).catch(e=>createCustomAlert(e.message,"Fehler"))}createExplanation(){let e=document.createElement("div");e.classList.add("explanation-container"),e.innerHTML="\n            <h2>Hotkeys/Schaltflächen</h2>\n            <p><b>Speichern</b> - [<b>strg</b>] + [<b>s</b>]:\n                <p>Sichert die Eingabe des aktiven Formulars in der Datenbank. Vor dem Speichern werden validitäts Prüfungen durchgeführt.</p>\n                <p>Speichern kann durch fehlende Verbindung zur Datenbank oder durch invalide Eingabe fehlschlagen.</p>\n            </p>\n            <p><b>Neu Anlegen</b> - [<b>strg</b>] + [<b>einfg</b>]:\n                <p>Öffnet ein neues, leeres Formular ohne ID. Beim ersten Speichern wird eine neue ID generiert.</p>\n                <p>Schließt das aktive Formular. Eingaben sollten vorher gespeichert werden.</p>\n            </p>\n            <p><b>Löschen</b> - [<b>strg</b>] + [<b>entf</b>]:\n                <p>Löscht das aktive Formular und markiert den entsprechenden Eintrag in der Datenbank als gelöscht. Es kann noch immer geöffnet und bearbeitet werden, taucht jedoch nicht mehr in der Übersicht auf.</p>\n                <p>Das Wiederherstellen eines gelöschten Formulars kann, momentan, nur durch manuelles Ändern der Löschmarkierung in der Datenbank rückgängig gemacht werden. Wenn nötig bei <b>r.seidler</b> melden.</p>\n            </p>\n            <p><b>Kopieren</b> - [<b>strg</b>] + [<b>k</b>]:\n                <p>Erstellt ein neues Formular mit neuer ID, übernimmt aber alle Eingaben des aktuellen Formulars.</p>\n                <p>Schließt das aktive Formular. Eingaben sollten vorher gespeichert werden.</p>\n            </p>\n            <br/>\n            <h2>Eingabe Historie</h2>\n            <p><b>Geplante Funktionsweise</b>: \n                <p>Alle Text-Eingabe-Felder haben einen Nutzer bzw. Browser Spezifischen Speicher vorangegangener Eingaben. Bei jedem Speichern werden dabei alle Eingaben für jedes Feld individuell im Browser gesichert.</p>\n                <p>Beim Fokusieren eines Text-Eingabe-Feldes sollte sich ein Dropdown-Menü mit vorigen Eingaben öffnen (falls vorhanden).</p>\n                <p>Mit den Pfeiltasten [hoch] und [runter] lässt sich zwischenn Einträgen wechseln. [Enter] überträgt die aktuelle Auswahl ins Eingabe-Feld. [Esc] oder Fokuswechsel sollte das Dropdown-Menü schließen.</p>\n                <p>Alternativ können Einträge mit der Maus ausgewählt werden.</p>\n                <p>Mit [entf] kann der aktuell markierte Eintrag gelöscht werden.</p>\n                <p>Nach einer Zeichen-Eingabe wird die Auswahl-Liste entsprechend der Eingabe gefiltert.</p>\n            </p>\n            <p><b>Alternativer Modus</b>:\n                <p>Auf Anfrage bei <b>r.seidler</b> können Eingabe-Felder auf die Nutzung von Einträgen aus der Datenbank umgeschalten werden. Die Eingabe-Vorschläge sind dann alle momentan für das entsprechende Feld gespeicherten Eintäge.</p>\n                <p>Allerdings lassen sich dann für diese Felder keine individuellen Anpassungen mehr vornehmen (eg. Einträge Löschen).</p>\n                <p>Als Beispiel nutzt das <b>Artikelnummer</b> Feld vorerst diesen Modus.</p>\n            </p>\n            <br/>\n            \n        ",this.append(e)}loadFromLocal(e){return e||(e=null===(e=new SearchParams(location.search).get("lsid"))?"last":e),this.convertValue("initialModel",localStorage.getItem(e))}saveFormLocal(e,t){let n=this.saveValue("model",t);console.group("saving model to localStorage"),e&&(localStorage.setItem(`${e}-${this.schema.formular}`,n),console.log("saved as:",`${e}-${this.schema.formular}`));let o=parseInt(localStorage.getItem("last-x"));Number.isNaN(o)||o>=10?(localStorage.setItem("last-x",0),localStorage.setItem("last-0",n),console.log("saved as:","last-0")):(localStorage.setItem("last-x",o+1),localStorage.setItem(`last-${o+1}`,n),console.log("saved as:",`last-${o+1}`)),localStorage.setItem("last",n),console.log("saved as:","last"),console.groupEnd()}}customElements.define("prot-form-gen",FormCreator);
...
const{InputFieldText:InputFieldText,InputFieldEnumListText:InputFieldEnumListText,EnumListableMixin:EnumListableMixin,InputFieldEmail:InputFieldEmail,InputFieldTel:InputFieldTel,InputFieldDate:InputFieldDate,InputFieldNumber:InputFieldNumber}=require("./input-field-generic.js"),{InputFieldTextarea:InputFieldTextarea}=require("./input-field-textarea.js"),{InputFieldBoolean:InputFieldBoolean}=require("./input-field-boolean.js"),{InputFieldDropdown:InputFieldDropdown}=require("./input-field-dropdown.js"),{InputFieldRadio:InputFieldRadio}=require("./input-field-radio.js"),{InputFieldLookup:InputFieldLookup}=require("./input-field-lookup.js"),{InputFieldAbhaengig:InputFieldAbhaengig,DependenceMixin:DependenceMixin}=require("./dependent-fields.js"),{InputFieldList:InputFieldList}=require("./input-field-list.js"),{InputFieldObject:InputFieldObject}=require("./input-field-object.js"),{InputFieldChooseList:InputFieldChooseList}=require("./input-field-choose-list.js"),{InfoFieldSummary:InfoFieldSummary}=require("./info-field-summary.js"),InputFieldDependentEnumTextarea=class extends(EnumListableMixin(DependenceMixin(InputFieldTextarea))){constructor(){super()}},InputFieldEnumTextArea=class extends(EnumListableMixin(InputFieldTextarea)){constructor(){super()}},InputFieldDependentChooseList=class extends(DependenceMixin(InputFieldChooseList)){constructor(){super()}},fieldTypeMap=module.exports.fieldTypeMap={text:{tag:"input-field-text",conName:InputFieldText},enumtext:{tag:"input-field-enumlisttext",conName:InputFieldEnumListText},enumtextarea:{tag:"input-field-enumlisttextarea",conName:InputFieldEnumTextArea},dependentenumtextarea:{tag:"input-field-dependentenumtextarea",conName:InputFieldDependentEnumTextarea},email:{tag:"input-field-email",conName:InputFieldEmail},tel:{tag:"input-field-tel",conName:InputFieldTel},date:{tag:"input-field-date",conName:InputFieldDate},number:{tag:"input-field-number",conName:InputFieldNumber},textarea:{tag:"input-field-textarea",conName:InputFieldTextarea},boolean:{tag:"input-field-boolean",conName:InputFieldBoolean},dropdown:{tag:"input-field-dropdown",conName:InputFieldDropdown},radio:{tag:"input-field-radio",conName:InputFieldRadio},lookup:{tag:"input-field-lookup",conName:InputFieldLookup},dependenttext:{tag:"input-field-abhaengig",conName:InputFieldAbhaengig},list:{tag:"input-field-list",conName:InputFieldList},object:{tag:"input-field-object",conName:InputFieldObject},choose:{tag:"input-field-choose-list",conName:InputFieldChooseList},dependentchoose:{tag:"input-field-dependent-choose-list",conName:InputFieldDependentChooseList},infosummary:{tag:"info-field-summary",conName:InfoFieldSummary}};
...
function generateRandomString(){let e=Math.floor(10*Math.random())+1;return[...new Array(e)].map(()=>String.fromCharCode(Math.floor(26*Math.random()+97))).join("")}function removeDuplicates(e){let t=[];return e.forEach(e=>{t.includes(e)||t.push(e)}),t}class HistoryInputExtender extends HTMLElement{constructor(){super()}connectedCallback(){if(this.historySource=this.getAttribute("history-source")||"local",this.historyInputSelector=this.getAttribute("history-input-selector")||"input",this.historyInputResultAttr=this.getAttribute("history-input-result-attr")||"value",this.elements={root:document.querySelector("prot-form-gen")||{schema:{formular:"test_form"},modelId:100},in:this.parentElement.querySelector(this.historyInputSelector),ul:void 0,li:[]},this.storageName=`${this.elements.root.schema.formular}.${this.elements.root.modelId}.${this.elements.in.id}`,this.markedItem=-1,!this.historyInputResultAttr in this.elements.in)throw new Error(`Referenced ELement has no ${this.historyInputResultAttr} attribute;`);this.obtainHistorySource()}obtainHistorySource(){"local"===this.historySource?(this.history=this.filteredHistory=JSON.parse(localStorage.getItem(this.storageName)||"[]"),this.createHistory()):"global"===this.historySource&&window.addEventListener("message",this.onMessageSourceListener.bind(this,this.createHistory))}createHistory(){this.createDropdown(),this.setupListeners()}applyFilter(){this.filteredHistory=this.history.filter(e=>Boolean(e)).filter(e=>e.toLowerCase().startsWith(this.elements.in[this.historyInputResultAttr].toLowerCase())),this.createListElements()}createDropdown(){this.isVisible=!1,this.className="history hidden",this.elements.ul=document.createElement("ul"),this.createListElements(),this.append(this.elements.ul)}showDropdown(){this.classList.remove("hidden"),this.isVisible=!0}hideDropdown(){this.classList.add("hidden");let e=document.querySelector("li.marked");e&&e.classList.remove("marked"),this.markedItem=-1,this.isVisible=!1}chooseEntry(e){this.elements.in[this.historyInputResultAttr]=e,this.elements.in.dispatchEvent(new Event("input",{bubbles:!0,cancelable:!0})),this.hideDropdown()}createListElements(e){this.elements.li=[],this.elements.ul.innerHTML="",this.markedItem=this.markedItem>-1?e||0:-1,this.filteredHistory.forEach((e,t)=>{let s=document.createElement("li");this.elements.li.push(s),t===this.markedItem&&s.classList.add("marked"),s.innerText=e,this.elements.ul.append(s)})}moveMarkedItem(e,t){-1!==e&&this.elements.li[e].classList.remove("marked"),-1===e?this.markedItem=-1===t?this.filteredHistory.length-1:0:1===this.filteredHistory.length?this.markedItem=0===e?-1:0:0===e?this.markedItem=-1===t?-1:e+t:e===this.filteredHistory.length-1?this.markedItem=-1===t?e+t:-1:this.markedItem=e+t,-1!==this.markedItem&&(this.elements.li[this.markedItem].classList.add("marked"),this.elements.ul.scrollTo(0,this.elements.li[this.markedItem].offsetTop))}onClickListener(e){"LI"===e.target.nodeName&&(e.preventDefault(),this.chooseEntry(e.target.innerText))}onFocusListener(e){this.applyFilter(),this.filteredHistory.length>0&&this.showDropdown()}onBlurListener(e){this.hideDropdown()}onMouseDownListener(e){e.preventDefault()}onKeyListener(e){13==e.which&&-1!==this.markedItem&&this.chooseEntry(this.filteredHistory[this.markedItem]),27==e.which&&this.isVisible&&(this.hideDropdown(),e.preventDefault()),38==e.which&&this.filteredHistory.length>0&&(this.moveMarkedItem(this.markedItem,-1),this.showDropdown(),e.preventDefault()),40==e.which&&this.filteredHistory.length>0&&(this.moveMarkedItem(this.markedItem,1),this.showDropdown(),e.preventDefault()),e.ctrlKey||46!=e.which||"local"!==this.historySource||-1!==this.markedItem&&(this.removeValue(this.filteredHistory[this.markedItem]),0===this.history.length&&this.hideDropdown(),e.preventDefault())}onInputListener(e){this.applyFilter()}onMessageListener(e){try{let t=JSON.parse(e.data);"submit-msg"===t.messageType&&"local"===this.historySource?this.recordValue(t.messageData[this.elements.in.id]):"clear-storage"===t.messageType&&"local"===this.historySource&&t.messageData[this.elements.in.id]&&localStorage.setItem(this.storageName,"[]")}catch(e){}}onMessageSourceListener(e,t){let s=JSON.parse(t.data);"history-source-models"===s.messageType&&(this.history=this.filteredHistory=removeDuplicates(s.messageData.map(e=>e[this.elements.in.id])),e.bind(this)())}setupListeners(){this.elements.ul.addEventListener("click",this.onClickListener.bind(this)),this.elements.ul.addEventListener("mousedown",this.onMouseDownListener.bind(this)),this.elements.in.addEventListener("focus",this.onFocusListener.bind(this)),this.elements.in.addEventListener("blur",this.onBlurListener.bind(this)),this.elements.in.addEventListener("keydown",this.onKeyListener.bind(this)),this.elements.in.addEventListener("input",this.onInputListener.bind(this)),window.addEventListener("message",this.onMessageListener.bind(this))}recordValue(e){if(console.log({operation:"record",element:this.elements.in.id,value:e}),!Boolean(e))return;-1===this.history.findIndex(t=>e.toLowerCase()===t.toLowerCase())&&this.history.push(e),localStorage.setItem(this.storageName,JSON.stringify(this.history))}removeValue(e){console.log({operation:"remove",element:this.elements.in.id,value:e}),this.history=this.history.filter(t=>t.toLowerCase()!==e.toLowerCase()),this.filteredHistory=this.history.filter(e=>Boolean(e)).filter(e=>e.toLowerCase().startsWith(this.elements.in[this.historyInputResultAttr].toLowerCase())),this.markedItem===this.filteredHistory.length&&(this.markedItem-=1),localStorage.setItem(this.storageName,JSON.stringify(this.history)),this.createListElements(this.markedItem)}}customElements.define("history-input-extender",HistoryInputExtender);

},{}],6:[function(require,module,exports){
const{InputField:InputField}=require("./input-field.js");module.exports.InfoFieldSummary=class extends InputField{constructor(){super(),this.defaultOptions={...this.defaultOptions,observed_fields:[]}}countFields(){let e={};return this.options.observed_fields.forEach(t=>{document.querySelectorAll(`#${t}`).forEach(t=>{let n=t.parentElement.parentElement.getModel();n&&n.forEach(t=>{e[t]=void 0===e[t]?1:e[t]+1})})}),e}runCounting(){let e=this.countFields();this.resultList.innerHTML=`${Object.keys(e).map(t=>`<li>${e[t]} x <span class="span-fett">${t}</span></li>`).join("")}`}applyTemplate(){this.rootElement.insertAdjacentHTML("beforeend",`\n          <div class="form-element">\n              ${this.options.label?`<label for="${this.options.name}">${this.options.label}</label><br>`:""}\n              <ul id=${this.options.name}></ul>\n          </div>\n      `),this.resultList=this.querySelector(`#${this.options.name}`),document.querySelector(".form-root").addEventListener("form-valid",this.runCounting.bind(this)),setTimeout(this.runCounting.bind(this),1e3)}getModel(){}};

},{"./input-field.js":16}],7:[function(require,module,exports){
const{InputField:InputField}=require("./input-field.js");module.exports.InputFieldBoolean=class extends InputField{constructor(){super(),this.defaultOptions={...this.defaultOptions,standard:!1}}applyTemplate(){this.rootElement.insertAdjacentHTML("beforeend",`\n          <div class="form-element">\n              ${this.options.label?`<label for="${this.options.name}">${this.options.label}</label><br>`:""}\n              <input  \n                  id="${this.options.name}" \n                  ${this.options.initialModel?"checked":"unchecked"}\n                  ${this.options.deaktiviert?"disabled":""}\n                  type="checkbox"\n                  title="${this.options.beschreibung}"\n              >\n              <span class="pflichtfeld" style="font-style: italic; visibility: ${this.options.pflichtfeld?"visible":"hidden"};">Pflichtfeld</span>\n          </div>\n      `),this.querySelector("input").addEventListener("input",t=>(console.log("checked",t),this.dispatchCustomEvent.bind(this,"form-input")(t)))}getModel(){return this.querySelector(`#${this.options.name}`).checked}};

},{"./input-field.js":16}],8:[function(require,module,exports){
const{InputFieldText:InputFieldText}=require("./input-field-generic.js"),{genericLookUpQuery:genericLookUpQuery}=require("./input-field-lookup.js");module.exports.InputFieldChooseList=class extends InputFieldText{constructor(){super(),this.defaultOptions={...this.defaultOptions,queryUrl:"",listenQuery:"",formWert:""},this.orig_list_items=[]}applyTemplate(){super.applyTemplate(),this.querySelector(".form-element").insertAdjacentHTML("afterbegin",'\n      <input \n          class="filter-input helper"\n          type="text"\n          placeholder="Filter"\n      >\n      <ul class="choose-list">\n\n      </ul>\n    '),this.querySelector(".form-element").insertAdjacentHTML("afterend",'<div class="separator"></div>'),console.log(this.options.listenQuery),genericLookUpQuery(this.options.queryUrl,"",this.options.listenQuery).then(e=>{let t=this.querySelector(`input#${this.options.name}`);this.createChooseList(e,t),this.initialSelect(t),t.addEventListener("input",i=>{let s=i.target.parentElement;if(""===i.target.value){let e=s.querySelector(".selected-item"),t=s.querySelector(".selected");e&&t&&(e.classList.remove("selected-item"),t.classList.remove("selected"))}s.parentElement.createChooseList(e,t)})}).catch(e=>{console.log("Database could not be reached?"),console.error(e),this.dbfailed=!0}),this.getModel()&&this.formInputHandler({target:this})}initialSelect(e){this.querySelector(".choose-list").innerHTML=""===e.value?this.orig_list_items.join("\n"):this.orig_list_items.filter(t=>t.toLowerCase().includes(e.value)).join("\n"),this.checkValidity()}createChooseList(e,t){this.dbfailed=!1,this.orig_list_items=e.map((e,t)=>`<li class="${t%2==0?"zebra1":"zebra2"}" onclick="(function(event){ console.log(event.target); let compo = event.target.parentElement.parentElement.parentElement; console.log(compo); compo.querySelector('#${this.options.name}').value = event.target.value; compo.querySelector('ul').classList.add('selected-item'); compo.formInputHandler({target: compo}); compo.dispatchCustomEvent('form-input', event)})(event)" value="${e[this.options.formWert]}">${Object.keys(e).map(t=>e[t]).join(", ")}</li>\n    `);let i=this.querySelector(".choose-list"),s=this.querySelector(".filter-input");i.innerHTML=""===t.value?this.orig_list_items.join("\n"):this.orig_list_items.filter(e=>e.toLowerCase().includes(t.value)).join("\n"),s.addEventListener("input",e=>i.innerHTML=this.orig_list_items.filter(t=>t.toLowerCase().includes(e.target.value)).join("\n"))}checkValidity(){let e=super.checkValidity(),t=this.getModel();[...this.querySelectorAll("li")].forEach(e=>e.classList.remove("selected"));let i=this.querySelector(`li[value="${t}"]`);return i?(i.classList.add("selected"),this.querySelector("ul").classList.add("selected-item")):(console.log(t),this.dbfailed||void 0===t||this.setValidityStatus(!1,"Kein entsprechender Wert in DB gefunden. Lösch die Eingabe und wähl ein Element aus der Liste aus")),this.dbfailed&&e&&this.setValidityStatus(!0,"Datenbank nicht erreichbar.",!0),e&&(this.dbfailed||i)}};

},{"./input-field-generic.js":10,"./input-field-lookup.js":12}],9:[function(require,module,exports){
const{InputField:InputField}=require("./input-field.js");module.exports.InputFieldDropdown=class extends InputField{constructor(){super(),this.defaultOptions={...this.defaultOptions,items:[]}}applyTemplate(){this.rootElement.insertAdjacentHTML("beforeend",`\n          <div class="form-element">\n              ${this.options.label?`<label for="${this.options.name}">${this.options.label}</label><br>`:""}\n              <select \n                  id="${this.options.name}" \n                  ${this.options.deaktiviert?"disabled":""}\n              >\n                  \n                  ${this.options.items.map(e=>`<option value="${e}" ${this.options.initialModel===e?"selected":""}>${e}</option>`).join("/n")}\n              </select>\n              <span class="pflichtfeld" style="font-style: italic; visibility: ${this.options.pflichtfeld?"visible":"hidden"};">Pflichtfeld</span>\n          </div>\n      `)}getModel(){return this.querySelector("select").value}};

},{"./input-field.js":16}],10:[function(require,module,exports){
const{InputField:InputField}=require("./input-field.js");class GenericInputField extends InputField{constructor(t){super(),this.defaultOptions={...this.defaultOptions,platzhalter:"",muster:".*",inputType:t,history:"local"}}applyTemplate(){this.rootElement.insertAdjacentHTML("beforeend",`\n          <div class="form-element">\n              ${this.options.label?`<label for="${this.options.name}">${this.options.label}</label><br>`:""}\n              <input \n                  id="${this.options.name}" \n                  placeholder="${this.options.platzhalter}" \n                  ${this.options.initialModel?`value="${this.options.initialModel}"`:""}\n                  pattern="${this.options.muster}" \n                  ${this.options.deaktiviert?"disabled":""}\n                  type="${this.options.inputType}"\n                  title="${this.options.beschreibung}"\n              >\n              ${"none"!==this.options.history?`<history-input-extender history-source="${this.options.history}"></history-input-extender>`:""}\n              <span class="validity-message"></span>\n              <span class="pflichtfeld" style="font-style: italic; visibility: ${this.options.pflichtfeld?"visible":"hidden"};">Pflichtfeld</span>\n          </div>\n      `),this.querySelector(`#${this.options.name}`).addEventListener("input",this.dispatchCustomEvent.bind(this,"form-input"))}getModel(){let t=this.querySelector(`#${this.options.name}`),e=t?t.value:void 0;return""!=e?e.split("@").join("&#64;").split("'").join("&#39;"):void 0}checkValidity(){let t=super.checkValidity(!1),e=this.querySelector(`input#${this.options.name}`),i=e.validity.valid;return i?this.setValidityStatus(!0,""):this.setValidityStatus(!1,e.validationMessage),t&&i}}const InputFieldText=module.exports.InputFieldText=class extends GenericInputField{constructor(){super("text")}};module.exports.InputFieldEmail=class extends GenericInputField{constructor(){super("email")}},module.exports.InputFieldTel=class extends GenericInputField{constructor(){super("tel")}},module.exports.InputFieldDate=class extends GenericInputField{constructor(){super("date"),this.defaultOptions={...this.defaultOptions,standard:(new Date).toLocaleDateString("en-CA"),min:void 0,max:void 0}}},module.exports.InputFieldNumber=class extends GenericInputField{constructor(){super("number"),this.defaultOptions={...this.defaultOptions,min:0,max:void 0}}};const EnumListableMixin=module.exports.EnumListableMixin=(t=>(class extends t{constructor(){super(),this.defaultOptions={...this.defaultOptions,standard:[]}}applyTemplate(){this.options.initialModel=this.options.initialModel.join(", "),super.applyTemplate()}getModel(){let t=super.getModel();return t?t.split(",").map(t=>t.trim()):void 0}}));module.exports.InputFieldEnumListText=class extends(EnumListableMixin(InputFieldText)){constructor(){super()}};

},{"./input-field.js":16}],11:[function(require,module,exports){
const{InputField:InputField}=require("./input-field.js");module.exports.InputFieldList=class extends InputField{constructor(){super(),this.defaultOptions={...this.defaultOptions,standard:[],vorlage:[],hinzufuegenLabel:"+",entfernenLabel:"-"}}addListItemHandler(e){let t=e.srcElement.parentElement.parentElement,n=e.srcElement.previousElementSibling.childNodes.length?0:e.srcElement.previousElementSibling.childNodes.length,i=t.getElementTemplate("",n);e.srcElement.previousElementSibling.insertAdjacentHTML("beforeend",i)}applyTemplate(){this.rootElement.insertAdjacentHTML("beforeend",`\n          <div class="form-element">\n              ${this.options.label?`<label for="${this.options.name}">${this.options.label}</label><br>`:""}\n              <div class="form-list" id="${this.options.name}" ${this.options.deaktiviert?"disabled":""}>\n                  ${this.options.initialModel.length>0?this.options.initialModel.map((e,t)=>this.getElementTemplate(e,t)).join("\n"):""}\n              </div>\n              <button id="${this.options.name}-button" type="button" class="form-list-addbtn">${this.options.hinzufuegenLabel}</button>\n              <span class="pflichtfeld" style="font-style: italic; visibility: ${this.options.pflichtfeld?"visible":"hidden"};">Pflichtfeld</span>\n          </div>\n      `),this.querySelector("button.form-list-addbtn").addEventListener("click",this.addListItemHandler)}getElementTemplate(e,t){return`\n          ${this.options.vorlage.map(n=>`\n              <${this.mapFieldType(n.feldtyp)} \n                  ${Object.keys(n).map(e=>`${e}='${this.saveValue(e,n[e],t)}'`).join(" ")}\n                  initialModel='${this.saveValue("initialModel",e)}'\n              ></${this.mapFieldType(n.feldtyp)}>\n          `).join("\n")}\n          <button class="form-list-removebtn" tabIndex="-1" onclick="this.previousElementSibling.classList.add('delete-waiting'); if(confirm('sicher, dass der gewählte Eintrag gelöscht werden soll?')) (function(event){event.srcElement.previousElementSibling.remove(); event.srcElement.remove()})(event); else this.previousElementSibling.classList.remove('delete-waiting')" type="button">${this.options.entfernenLabel}</button>\n      `}saveValue(e,t,n){return"name"===e?JSON.stringify(`${t}-${n}`):JSON.stringify(t)}getModel(){let e=[];return this.querySelectorAll(`#${this.options.name} > :not(button)`).forEach(t=>{e.push(t.getModel())}),0===e.length?[]:e}checkValidity(){let e=!0;return this.querySelectorAll(`#${this.options.name} > :not(button)`).forEach(t=>{e=e&&t.checkValidity()}),e}};

},{"./input-field.js":16}],12:[function(require,module,exports){
const{InputFieldText:InputFieldText}=require("./input-field-generic.js"),{debounce:debounce}=require("lodash"),genericLookUpQuery=module.exports.genericLookUpQuery=function(t,e,i,s){return fetch(t,{method:"POST",body:JSON.stringify({query:i.split("?").join(`${e}`)}),headers:{"Content-Type":"application/json"}}).then(t=>t.json()).then(t=>(console.log(t),t))},LookupMixin=module.exports.LookupMixin=(t=>(class extends t{constructor(){super(),this.defaultOptions={...this.defaultOptions,ausgabeLabel:"",queryUrl:"",query:void 0,maxZeilen:1},this.setAsyncValidity(!0,"",!1)}databaseLookup(t,e){(e.target.validity?e.target.validity.valid:e.target.valid&&""!==e.target.value)?(console.log(this.options.query),genericLookUpQuery(this.options.queryUrl,t.bind(this)(),this.options.query).then(t=>t.map(t=>Object.keys(t).map(e=>`${e}: ${t[e]}`).join("<br>"))).then(t=>{t[0]?(this.querySelector(".output-display").innerHTML=t[0],this.setAsyncValidity(!0,"",!1)):this.setValidityStatus(!1,"Datenbank hat kein passendes Ergebniss zurück geliefert.",!0)}).catch(t=>{console.error(t),this.setAsyncValidity(!0,"Database could not be reached.",!0)})):(this.querySelector(".output-display").innerText="",this.valid=!1)}connectedCallback(){super.connectedCallback(),this.addLookupHandler()}addLookupHandler(){try{let t=debounce(this.databaseLookup,1e3,{trailing:!0,leading:!1,maxWait:5e3});this.querySelector("input").addEventListener("input",t.bind(this,this.getModel))}catch(t){console.log("probably no Input field. Fallback tp focusout."),console.error(t),this.addEventListener("focusout",this.databaseLookup.bind(this,this.getModel))}this.options.initialModel&&this.databaseLookup(this.getModel,{target:this}),this.querySelector(".form-element").insertAdjacentHTML("beforeend",'\n          <br><span class="output-display"></span>\n      ')}checkValidity(){return!(!super.checkValidity()||!this.checkAsyncValidity())&&(this.setValidityStatus(this.asyncValid,this.asyncValMessage,this.asyncWarning),!0)}checkAsyncValidity(){return!!this.asyncValid}setValidityStatus(t,e,i){return t||(this.querySelector(".output-display").innerText=""),super.setValidityStatus(t,e,i)}setAsyncValidity(t,e,i){t||(this.querySelector(".output-display").innerText=""),this.asyncValid=t,this.asyncWarning=i,this.asyncValMessage=e,this.setValidityStatus(t,e,i)}}));module.exports.InputFieldLookup=class extends(LookupMixin(InputFieldText)){constructor(){super()}};

},{"./input-field-generic.js":10,"lodash":18}],13:[function(require,module,exports){
const{InputField:InputField}=require("./input-field.js");module.exports.InputFieldObject=class extends InputField{constructor(){super(),this.defaultOptions={...this.defaultOptions,representant:"",subform:{}},this.collapsed=!1}collapseObjectGroupHandler(e){let t=e.srcElement,n=t.parentElement.parentElement,l=void 0,o="";try{l=n.getModel(),o=`${n.options.label} ${l?n.options.representant?l[n.options.representant]?l[n.options.representant]:"":Object.values(l)[0]:""}`}catch(e){console.log("SourceTree")}let i=n.querySelector(`label[for="${n.options.name}"]`),s=n.querySelector(`#${n.options.name}`);n.collapsed?(s.classList.remove("hidden"),t.innerText="einklappen",n.collapsed=!n.collapsed,i.innerText=n.options.label||""):(s.classList.add("hidden"),t.innerText="ausklappen",n.collapsed=!n.collapsed,i.innerText=o)}applyTemplate(){this.rootElement.insertAdjacentHTML("beforeend",`\n          <div class="form-element">\n              <button class="form-object-collapse" type="button" tabIndex="-1">einklappen</button>\n              <label for="${this.options.name}">${this.options.label||""}</label><br>\n              <div id="${this.options.name}" class="form-group">\n                  ${Object.keys(this.options.subform).map(e=>{let t=this.options.subform[e];return`\n                          <${this.mapFieldType(t.feldtyp)} \n                              ${Object.keys(t).map(e=>`${e}='${this.saveValue(e,t[e])}'`).join(" ")}\n                              ${this.options.initialModel&&this.options.initialModel[t.name]?`initialModel='${this.saveValue("initialModel",this.options.initialModel[t.name])}'`:""} \n                          ></${this.mapFieldType(t.feldtyp)}>`}).join("\n")}\n              </div>\n          </div>\n      `),this.querySelector("button.form-object-collapse").addEventListener("click",this.collapseObjectGroupHandler)}getModel(){let e={};return[...this.querySelector(`#${this.options.name}`).children].forEach(t=>{let n=t.getModel();n&&(e[JSON.parse(t.getAttribute("name"))]=n)}),0===Object.keys(e).length?void 0:e}checkValidity(){let e=!0;return[...this.querySelector(`#${this.options.name}`).children].forEach(t=>{let n=t.valid;e=e&&n}),e}};

},{"./input-field.js":16}],14:[function(require,module,exports){
const{InputField:InputField}=require("./input-field.js");module.exports.InputFieldRadio=class extends InputField{constructor(){super(),this.defaultOptions={...this.defaultOptions,abwahlButtonLabel:"Auswahl aufheben",items:[]},this.classList.add("radio-form")}radioButtonClearHandler(t){let e=t.target.parentElement.parentElement;e.querySelectorAll('input[type="radio"]').forEach(t=>t.checked=!1),e.dispatchCustomEvent("form-input",t)}applyTemplate(){this.rootElement.insertAdjacentHTML("beforeend",`\n            <div class="form-element">\n                <button type="button" class="clear-radio-btn" tabIndex="-1" >${this.options.abwahlButtonLabel}</button>\n                ${this.options.label?`<label for="${this.options.name}">${this.options.label}</label>`:""}\n                <form name="${this.options.name}-radio" id="${this.options.name}" class="form-radio-group"><br>\n                    ${this.options.items.map(t=>`\n                                <input type="radio" id="${this.options.name}-${t}" name="${this.options.name}" value="${t}" ${this.options.initialModel===t?"checked":""}>\n                                <label for="${this.options.name}-${t}">${t}</label>\n                            `).join("<br>")}\n                    <span class="validity-message"></span>\n                    <span class="pflichtfeld" style="font-style: italic; visibility: ${this.options.pflichtfeld?"visible":"hidden"};">Pflichtfeld</span>\n                </form>\n            </div>\n        `),this.querySelector("button.clear-radio-btn").addEventListener("click",this.radioButtonClearHandler),this.querySelectorAll("input").forEach(t=>t.addEventListener("input",this.dispatchCustomEvent.bind(this,"form-input")))}getModel(){let t=void 0;return this.querySelectorAll('input[type="radio"]').forEach(e=>{e.checked&&(t=e.value)}),t}checkValidity(){return!this.getModel()&&this.options.pflichtfeld?(this.setValidityStatus(!1,"Dies ist ein Pflichtfeld.",!1),!1):(this.setValidityStatus(!0,"",!1),!0)}};

},{"./input-field.js":16}],15:[function(require,module,exports){
const{InputField:InputField}=require("./input-field.js");module.exports.InputFieldTextarea=class extends InputField{constructor(){super(),this.defaultOptions={...this.defaultOptions,platzhalter:"",cols:50,rows:5,wrap:"soft",history:"local"}}applyTemplate(){this.rootElement.insertAdjacentHTML("beforeend",`\n          <div class="form-element">\n              ${this.options.label?`<label for="${this.options.name}">${this.options.label}</label><br>`:""}\n              <textarea \n                  id="${this.options.name}" \n                  placeholder="${this.options.platzhalter}"  \n                  ${this.options.deaktiviert?"disabled":""}\n                  cols="${this.options.cols}"\n                  rows="${this.options.rows}"\n                  wrap="${this.options.wrap}"\n                  title="${this.options.beschreibung}"\n              >${this.options.initialModel?this.options.initialModel:""}</textarea>\n              ${"none"!==this.options.history?`<history-input-extender history-source="${this.options.history}" history-input-selector="textarea"></history-input-extender>`:""}\n              <span class="validity-message"></span>\n              <span class="pflichtfeld" style="font-style: italic; visibility: ${this.options.pflichtfeld?"visible":"hidden"};">Pflichtfeld</span>\n          </div>\n      `),this.querySelector("textarea").addEventListener("input",this.dispatchCustomEvent.bind(this,"form-input"))}getModel(){let t=this.querySelector(`#${this.options.name}`),i=t?t.value:void 0,e=""!=i?i.split("@").join("&#64;").split("'").join("&#39;"):void 0;if(!this.options.nomodel)return e;localStorage.setItem(`${this.options.nomodel_unique_id}|${this.options.name}|${this.model["#modelID"]}`,e)}};

},{"./input-field.js":16}],16:[function(require,module,exports){
const{debounce:debounce}=require("lodash"),fieldTypeMap={text:"input-field-text",enumtext:"input-field-enumlisttext",enumtextarea:"input-field-enumlisttextarea",dependentenumtextarea:"input-field-dependentenumtextarea",email:"input-field-email",tel:"input-field-tel",date:"input-field-date",number:"input-field-number",textarea:"input-field-textarea",boolean:"input-field-boolean",dropdown:"input-field-dropdown",radio:"input-field-radio",lookup:"input-field-lookup",dependenttext:"input-field-abhaengig",list:"input-field-list",object:"input-field-object",choose:"input-field-choose-list",dependentchoose:"input-field-dependent-choose-list",infosummary:"info-field-summary"};module.exports.InputField=class extends HTMLElement{constructor(){super(),this.schema={},this.template="",this.defaultOptions={initialModel:"",name:"",label:"",beschreibung:"",standard:"",deaktiviert:!1,pflichtfeld:!1,hintergrundFarbe:"none"},this.rootElement=this,this.options={},this.model={},this.valid=!0,this.validityMessage=void 0,this.addEventListener("form-input",debounce(this.formInputHandler,1e3,{leading:!1,trailing:!0}))}connectedCallback(){Object.keys(this.defaultOptions).forEach(t=>{this.options[t]=this.convertValue(t,this.getAttribute(t))||this.defaultOptions[t]}),null!=this.options.initialModel&&""!=this.options.initialModel||(this.options.initialModel=this.options.standard),""===this.options.beschreibung&&(this.options.beschreibung=this.options.name),this.applyTemplate(),this.querySelector(".form-element").style.backgroundColor=this.options.hintergrundFarbe}applyTemplate(){throw Error("Not Implemented")}convertValue(t,e){try{return null!=e?JSON.parse(e):""}catch(i){console.error(i),console.log(t,e.toSource(),typeof e)}}saveValue(t,e){return"query"!==t&&"listenQuery"!==t||(e=e.split("'").join("&#39;")),JSON.stringify(e)}checkValidity(){return this.options.pflichtfeld&&null==this.getModel()?(this.setValidityStatus(!1,"Dies ist ein Pflichtfeld, und muss ausgefüllt werden."),!1):(this.setValidityStatus(!0,""),!0)}setValidityStatus(t,e,i){this.valid=t,this.validityMessage=e,this.setAttribute("data-tooltip",e);let s=this.querySelector(".validity-message");t&&!i||!s||(s.innerText=e),t?(this.classList.remove("invalid"),i||this.classList.remove("warning")):this.classList.add("invalid"),i&&this.classList.add("warning")}setModel(){throw Error("Not Implemented")}getModel(){throw Error("Not Implemented")}mapFieldType(t){return fieldTypeMap[t]}dispatchCustomEvent(t,e){return this.dispatchEvent(new Event(t,{bubbles:!0,target:this,value:this.getModel()}))}formInputHandler(t){t.target.checkValidity()?this.dispatchCustomEvent("form-valid",{target:this}):this.dispatchCustomEvent("form-invalid",{target:this})}};

},{"lodash":18}],17:[function(require,module,exports){
const url="http://theia.protronic-gmbh.de:4040/";function sendLogToLogstash(t){fetch(url,{method:"POST",body:JSON.stringify(t),headers:{"Content-Type":"application/json"}}).then(t=>{console.log({status:t.status,statusText:t.statusText})})}module.exports={sendLogToLogstash:sendLogToLogstash};
