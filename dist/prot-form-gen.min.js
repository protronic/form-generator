                ...
                return r
                    ...
                    1: [function(require, module, exports) {
                        ...
                        function createCustomAlert(e, t) {
                            if (null == t && (t = "Fehler"), document.getElementById("modalContainer")) return;
                            let n = document.body.appendChild(document.createElement("div"));
                            n.id = "modalContainer", n.style.height = document.documentElement.scrollHeight + "px";
                            let o = n.appendChild(document.createElement("div"));
                            o.id = "alertBox", o.classList.add(`${t.toLowerCase()}-alert`), document.all && !window.opera && (o.style.top = document.documentElement.scrollTop + "px"), o.style.left = (document.documentElement.scrollWidth - o.offsetWidth) / 2 + "px", o.appendChild(document.createElement("h1")).appendChild(document.createTextNode(t)), o.appendChild(document.createElement("p")).innerHTML = e;
                            let d = o.appendChild(document.createElement("a"));
                            d.id = "closeBtn", d.appendChild(document.createTextNode(ALERT_BUTTON_TEXT)), d.href = "#", d.onclick = function() { return removeCustomAlert(), !1 }, document.addEventListener("keydown", enterPressedListener)
                        }

                        function enterPressedListener(e) { if (13 == e.which) return document.removeEventListener("keydown", enterPressedListener), removeCustomAlert(), !1 }
                    ...
                    2: [function(require, module, exports) {
                        const { InputFieldText: InputFieldText } = require("./input-field-generic.js"), DependenceMixin = module.exports.DependenceMixin = (e => (class extends e {
                            constructor() { super(), this.defaultOptions = {...this.defaultOptions, abhaengigFeld: "", wertSichtbar: "", interval: 500 }, this.visibility = !0, this.hideField() }
                            applyTemplate() { super.applyTemplate(), this.linkDependency() }
                            checkDependence(e, i) { e.getModel() === this.options.wertSichtbar ? this.showField() : this.hideField() }
                            linkDependency() {
                                let e = this.parentElement.querySelector(`*[name='${this.saveValue("abhaengigFeld",this.options.abhaengigFeld)}']`);
                                e.addEventListener("form-valid", this.checkDependence.bind(this, e)), setInterval(this.checkDependence.bind(this, e), this.options.interval)
                            }
                            hideField() { this.visibility && (this.visibility = !1, this.classList.add("hidden")) }
                            showField() { this.visibility || (this.visibility = !0, this.classList.remove("hidden"), this.checkValidity()) }
                            getModel() { return this.visibility ? super.getModel() : void 0 }
                            checkValidity() { return !this.visibility || super.checkValidity() }
                        }));
                        module.exports.InputFieldAbhaengig = class extends(DependenceMixin(InputFieldText)) { constructor() { super() } };

                    }, { "./input-field-generic.js": 10 }],
                    3: [function(require, module, exports) {
                        require("./history-input-extender.js");
                        const { InputFieldObject: InputFieldObject } = require("./input-field-object.js"), { fieldTypeMap: fieldTypeMap } = require("./formular-components.js"), { createCustomAlert: createCustomAlert } = require("./custom-alert-box.js"), { sendLogToLogstash: sendLogToLogstash } = require("./logging-connector.js");
                        var baseUrl = "http://10.19.28.94:8087",
                            schemaPath = "/schema",
                            modelPath = "/model";

                        function sendErrorReport(e, t, n, o) { let r = Object.assign({ appname: "json-form-creator", audit: !0, form: t, model: n, error: e, message: o }); throw sendLogToLogstash(r), new Error(e) }
                        Object.keys(fieldTypeMap).forEach(e => { customElements.define(fieldTypeMap[e].tag, fieldTypeMap[e].conName) });
                        class SearchParams {
                            constructor(e) {
                                let t = this;
                                e.slice(1).split("&").forEach(function(e) {
                                    let n = e.split("=")[0],
                                        o = e.split("=")[1];
                                    t[n] = o
                                })
                            ...
                            get(e) { return this[e] }
                            set(e, t) { this[e] = t }
                            delete(e) { delete this[e] }
                            toString() { return "?" + Object.keys(this).map(e => `${e}=${this[e]}`).join("&") }
                        }

                        function prepareModel(e, t) {
                            let n = "",
                                o = (new Date).getTime();
                            try { n = wikiContext.UserName } catch (e) { console.log("wikiContext not in scope.") }
                            return e["#parentForm"] = t, e["#changed_user"] = n, e["#changed_time"] = o, JSON.stringify(e)
                        }

                        function transformToHistoryModel(e) { let t = {}; return function e(t, n) { t && Object.keys(t).forEach(o => { "object" == typeof t[o] ? e(t[o], n) : n[o] = t[o] }) }(e, t), t }

                        function fetchGlobalHistoryModels(e) { fetch(`${baseUrl}${modelPath}?parentForm=${e}`).then(e => e.json()).then(e => e.map(e => e.log)).then(e => e.map(e => { try { return JSON.parse(e) } catch (e) {} })).then(e => e.map(e => transformToHistoryModel(e))).then(e => { console.log(e), window.postMessage(JSON.stringify({ messageType: "history-source-models", messageData: e })) }) }

                        function uploadNewModel(e, t) {
                            let n = prepareModel(e, t);
                            return fetch(`${baseUrl}${modelPath}`, { method: "POST", body: `${n}`, headers: { "Content-Type": "application/json" } }).then(n => {
                                if (n.ok) return n.json();
                                sendErrorReport(`Verbindung zur Datenbank ist Fehlgeschlagen mit status ${n.status} und folgender Meldung: ${n.statusText}.`, JSON.stringify(t), JSON.stringify(e), `URL: ${baseUrl}${modelPath}}`)
                            }).then(e => (console.log(e), e._id))
                        }

                        function uploadExistingModel(e, t) {
                            let n = prepareModel(e, t);
                            return fetch(`${baseUrl}${modelPath}/${e["#modelID"]}`, { method: "POST", body: n, headers: { "Content-Type": "application/json" } }).then(n => {
                                if (n.ok) return n.json();
                                sendErrorReport(`Verbindung zur Datenbank ist Fehlgeschlagen mit status ${n.status} und folgender Meldung: ${n.statusText}.`, JSON.stringify(t), JSON.stringify(e), `URL: ${baseUrl}${modelPath}/${e["#modelID"]}`)
                            }).then(n => { 0 !== Object.keys(n).length && sendErrorReport("Speichern des Models ist aus einem unbekannten Grund fehlgeschlagen. Für mehr Informationen mit 'r.seidler' in Verbindung setzen.", JSON.stringify(t), JSON.stringify(e), `URL: ${baseUrl}${modelPath}/${e["#modelID"]}`), console.log(n) })
                        }

                        function removeExistingModel(e, t) { let n = e["#modelID"]; return fetch(`${baseUrl}${modelPath}/${n}`, { method: "DELETE" }).then(o => { o.ok || sendErrorReport(`Beim Löschen des models hat die Datenbank einen Fehlercode zurückgegeben. Fehlercode: ${o.status}; Fehlernachricht: ${o.statusText}`, JSON.stringify(t), JSON.stringify(e), `URL: ${baseUrl}${modelPath}/${n}`) }) }

                        function loadModelFromDB(e, t) {
                            return fetch(`${baseUrl}${modelPath}/${e}`).then(n => {
                                if (n.ok) return n.json();
                                sendErrorReport(`Beim Laden des models hat die Datenbank einen Fehlercode zurückgegeben. Fehlercode: ${n.status}; Fehlernachricht: ${n.statusText}`, JSON.stringify(t), "", `URL: ${baseUrl}${modelPath}/${e}`)
                            }).then(t => (console.log({ model: t }), t["#modelID"] = e, t))
                        }

                        function loadSchemaFromDB(e) {
                            if (null != e) return fetch(`${baseUrl}${schemaPath}/${e}`).then(t => {
                                if (t.ok || 500 == t.status) return t.json();
                                sendErrorReport(`Datenbank Anfrage gab Fehlercode ${t.status}, mit folgender Nachricht zurück: "${t.statusText}".`, "", "", `URL: ${baseUrl}${schemaPath}/${e}`)
                            }).then(t => {
                                if (null == t.error) return t;
                                sendErrorReport(`Schema #${e} konnte in der Datenbank nicht gefunden werden.`, "", "", `URL: ${baseUrl}${schemaPath}/${e}`)
                            });
                        ...
                        function getSchemaId() { return new SearchParams(location.search).get("schema") }
                        class FormCreator extends InputFieldObject {
                            constructor() { super(), this.model = {} }
                            connectedCallback() { this.rootElement = document.createElement("form"), this.rootElement.classList.add("form-root"), this.appendChild(this.rootElement), baseUrl = this.getAttribute("data-url") ? this.getAttribute("data-url") : baseUrl, loadSchemaFromDB(getSchemaId()).then(e => { this.applySchema(e) }).catch(e => { createCustomAlert(e.message) }), fetchGlobalHistoryModels() }
                            applySchema(e) {
                                console.log({ schema: e }), this.schema = e, this.options.name = e.formular, this.options.label = `${e.formular}`, this.options.subform = e.felder;
                                try {
                                    let t = this.modelId = new SearchParams(location.search).get("mid");
                                    t ? loadModelFromDB(t, e.formular).then(n => { this.model = this.convertValue("initialModel", JSON.stringify(n)), this.model["#modelID"] = t, this.options.name = e.formular, this.options.label = `${this.options.label} ${t}`, this.options.initialModel = this.convertValue("initialModel", JSON.stringify(n)), this.rootElement.options = this.options, this.applyTemplate() }).catch(e => createCustomAlert(e.message, "Fehler")) : (this.options.initialModel = this.loadFromLocal(), this.rootElement.options = this.options, this.applyTemplate())
                                } catch (e) { console.log(this.options.initialModel), console.error(e) }
                                this.createButtonContainer(), this.createExplanation(), this.renderGeneralInfo(e)
                            }
                            renderGeneralInfo(e) { this.insertAdjacentHTML("afterbegin", `\n            <div class="formular-title">\n                <h2>${e.formular}</h2>\n            </div>\n        `) }
                            testModelCreation() {
                                let e = document.createElement("button");
                                e.setAttribute("type", "button"), e.addEventListener("click", e => { this.checkValidity() ? (this.model = {...this.model, ...this.getModel() }, this.saveFormLocal(this.model["#modelID"], this.model)) : console.error("Einige Formular Elemente sind nicht korrekt ausgefüllt.") }), e.innerText = "Erzeuge Model", this.appendChild(e)
                            }
                            createButtonContainer() { this.buttonContainer = document.createElement("div"), this.buttonContainer.classList.add("btn-container"), this.appendChild(this.buttonContainer), this.uploadModelButton(), this.newFormularButton(), this.removeButton(), this.copyModelButton() }
                            createNewFormURL(e) {
                                let t = new URL(location.href),
                                    n = new SearchParams(location.search);
                                return n.set("lsid", "null"), n.delete("mid"), void 0 !== e ? (n.set("mid", e), t.search = n.toString(), history.pushState({ schema: n.get("schema"), mid: e }, "", t.toString())) : (t.search = n.toString(), history.pushState({ schema: n.get("schema") }, "", t.toString())), t.href
                            }
                            newFormularButton() {
                                let e = document.createElement("button");
                                e.setAttribute("type", "button"), e.addEventListener("click", this.newFormularButtonClickListener.bind(this)), document.addEventListener("keydown", this.newFormularButtonHotkeyListener.bind(this)), e.innerText = "Neu Anlegen", this.buttonContainer.appendChild(e)
                            }
                            newFormularButtonHotkeyListener(e) { e.ctrlKey && 45 == e.which && (this.newFormularButtonClickListener.bind(this, void 0)(), e.preventDefault()) }
                            newFormularButtonClickListener(e) { this.createNewFormURL(), this.remove(), document.body.append(document.createElement("prot-form-gen")) }
                            removeButton() {
                                let e = document.createElement("button");
                                e.setAttribute("type", "button"), e.addEventListener("click", this.removeButtonClickListener.bind(this)), document.addEventListener("keydown", this.removeButtonHotkeyListener.bind(this)), e.innerText = "Löschen", this.buttonContainer.appendChild(e)
                            }
                            removeButtonHotkeyListener(e) { e.ctrlKey && 46 == e.which && (this.removeButtonClickListener.bind(this, void 0)(), e.preventDefault()) }
                            removeButtonClickListener(e) { this.model["#modelID"] && confirm(`Are you sure, you want to renove model with id: ${this.model["#modelID"]}?`) && removeExistingModel(this.model, this.schema.formular).then(() => { this.createNewFormURL(), this.remove(), document.body.append(document.createElement("prot-form-gen")) }).catch(e => createCustomAlert(e.message, "Fehler")) }
                            uploadModelButton() {
                                let e = document.createElement("button");
                                e.setAttribute("type", "button"), e.addEventListener("click", this.uploadModelButtonClickListener.bind(this)), document.addEventListener("keydown", this.uploadModelButtonHotkeyListener.bind(this)), e.innerText = "Speichern", this.buttonContainer.appendChild(e)
                            }
                            uploadModelButtonHotkeyListener(e) { e.ctrlKey && 83 == e.which && (this.uploadModelButtonClickListener.bind(this, void 0)(), e.preventDefault()) }
                            uploadModelButtonClickListener(e) { this.checkValidity() ? (this.model = {...this.getModel(), "#modelID": this.model["#modelID"] }, void 0 === this.model ? console.log("empty model") : (window.postMessage(JSON.stringify({ messageType: "submit-msg", messageData: transformToHistoryModel(this.model) })), this.saveFormLocal(this.model["#modelID"], this.model), this.model["#modelID"] ? (console.log(this.model), uploadExistingModel(this.model, this.schema.formular).then(() => createCustomAlert(`Änderungen an ${this.model["#modelID"]} wurden gespeichert.`, "Erfolg")).catch(e => createCustomAlert(e.message, "Fehler"))) : uploadNewModel(this.model, this.schema.formular).then(e => { this.model["#modelID"] = e, this.createNewFormURL(e), this.querySelector(`label[for="${this.schema.formular}"]`).innerText = `${this.options.label} ${e}`, createCustomAlert(`Daten wurden erfolgreich in der Datenbank unter folgender ID abgelegt.\n${e}`, "Erfolg") }).catch(e => createCustomAlert(e.message, "Fehler")))) : console.log("invalid") }
                            copyModelButton() {
                                let e = document.createElement("button");
                                e.setAttribute("type", "button"), e.addEventListener("click", this.copyModelButtonClickListener.bind(this)), document.addEventListener("keydown", this.copyModelButtonHotkeyListener.bind(this)), e.innerText = "Kopieren", this.buttonContainer.appendChild(e)
                            }
                            copyModelButtonHotkeyListener(e) { e.ctrlKey && 75 == e.which && (this.copyModelButtonClickListener.bind(this, void 0)(), e.preventDefault()) }
                            copyModelButtonClickListener(e) { this.model = this.getModel(), this.remove(), this.model["#modelID"] = void 0, uploadNewModel(this.model, this.schema.formular).then(e => { this.model["#modelID"] = e, location.href = this.createNewFormURL(e) }).catch(e => createCustomAlert(e.message, "Fehler")) }
                            createExplanation() {
                                let e = document.createElement("div");
                                e.classList.add("explanation-container"), e.innerHTML = "\n            <h2>Hotkeys/Schaltflächen</h2>\n            <p><b>Speichern</b> - [<b>strg</b>] + [<b>s</b>]:\n                <p>Sichert die Eingabe des aktiven Formulars in der Datenbank. Vor dem Speichern werden validitäts Prüfungen durchgeführt.</p>\n                <p>Speichern kann durch fehlende Verbindung zur Datenbank oder durch invalide Eingabe fehlschlagen.</p>\n            </p>\n            <p><b>Neu Anlegen</b> - [<b>strg</b>] + [<b>einfg</b>]:\n                <p>Öffnet ein neues, leeres Formular ohne ID. Beim ersten Speichern wird eine neue ID generiert.</p>\n                <p>Schließt das aktive Formular. Eingaben sollten vorher gespeichert werden.</p>\n            </p>\n            <p><b>Löschen</b> - [<b>strg</b>] + [<b>entf</b>]:\n                <p>Löscht das aktive Formular und markiert den entsprechenden Eintrag in der Datenbank als gelöscht. Es kann noch immer geöffnet und bearbeitet werden, taucht jedoch nicht mehr in der Übersicht auf.</p>\n                <p>Das Wiederherstellen eines gelöschten Formulars kann, momentan, nur durch manuelles Ändern der Löschmarkierung in der Datenbank rückgängig gemacht werden. Wenn nötig bei <b>r.seidler</b> melden.</p>\n            </p>\n            <p><b>Kopieren</b> - [<b>strg</b>] + [<b>k</b>]:\n                <p>Erstellt ein neues Formular mit neuer ID, übernimmt aber alle Eingaben des aktuellen Formulars.</p>\n                <p>Schließt das aktive Formular. Eingaben sollten vorher gespeichert werden.</p>\n            </p>\n            <br/>\n            <h2>Eingabe Historie</h2>\n            <p><b>Geplante Funktionsweise</b>: \n                <p>Alle Text-Eingabe-Felder haben einen Nutzer bzw. Browser Spezifischen Speicher vorangegangener Eingaben. Bei jedem Speichern werden dabei alle Eingaben für jedes Feld individuell im Browser gesichert.</p>\n                <p>Beim Fokusieren eines Text-Eingabe-Feldes sollte sich ein Dropdown-Menü mit vorigen Eingaben öffnen (falls vorhanden).</p>\n                <p>Mit den Pfeiltasten [hoch] und [runter] lässt sich zwischenn Einträgen wechseln. [Enter] überträgt die aktuelle Auswahl ins Eingabe-Feld. [Esc] oder Fokuswechsel sollte das Dropdown-Menü schließen.</p>\n                <p>Alternativ können Einträge mit der Maus ausgewählt werden.</p>\n                <p>Mit [entf] kann der aktuell markierte Eintrag gelöscht werden.</p>\n                <p>Nach einer Zeichen-Eingabe wird die Auswahl-Liste entsprechend der Eingabe gefiltert.</p>\n            </p>\n            <p><b>Alternativer Modus</b>:\n                <p>Auf Anfrage bei <b>r.seidler</b> können Eingabe-Felder auf die Nutzung von Einträgen aus der Datenbank umgeschalten werden. Die Eingabe-Vorschläge sind dann alle momentan für das entsprechende Feld gespeicherten Eintäge.</p>\n                <p>Allerdings lassen sich dann für diese Felder keine individuellen Anpassungen mehr vornehmen (eg. Einträge Löschen).</p>\n                <p>Als Beispiel nutzt das <b>Artikelnummer</b> Feld vorerst diesen Modus.</p>\n            </p>\n            <br/>\n            \n        ", this.append(e)
                            }
                            loadFromLocal(e) { return e || (e = null === (e = new SearchParams(location.search).get("lsid")) ? "last" : e), this.convertValue("initialModel", localStorage.getItem(e)) }
                            saveFormLocal(e, t) {
                                let n = this.saveValue("model", t);
                                console.group("saving model to localStorage"), e && (localStorage.setItem(`${e}-${this.schema.formular}`, n), console.log("saved as:", `${e}-${this.schema.formular}`));
                                let o = parseInt(localStorage.getItem("last-x"));
                                Number.isNaN(o) || o >= 10 ? (localStorage.setItem("last-x", 0), localStorage.setItem("last-0", n), console.log("saved as:", "last-0")) : (localStorage.setItem("last-x", o + 1), localStorage.setItem(`last-${o+1}`, n), console.log("saved as:", `last-${o+1}`)), localStorage.setItem("last", n), console.log("saved as:", "last"), console.groupEnd()
                            }
                        }
                    ...
                    5: [function(require, module, exports) {
                        function generateRandomString() { let e = Math.floor(10 * Math.random()) + 1; return [...new Array(e)].map(() => String.fromCharCode(Math.floor(26 * Math.random() + 97))).join("") }

                        function removeDuplicates(e) { let t = []; return e.forEach(e => { t.includes(e) || t.push(e) }), t }
                        class HistoryInputExtender extends HTMLElement {
                            constructor() { super() }
                            connectedCallback() {
                                if (this.historySource = this.getAttribute("history-source") || "local", this.historyInputSelector = this.getAttribute("history-input-selector") || "input", this.historyInputResultAttr = this.getAttribute("history-input-result-attr") || "value", this.elements = { root: document.querySelector("prot-form-gen") || { schema: { formular: "test_form" }, modelId: 100 }, in: this.parentElement.querySelector(this.historyInputSelector), ul: void 0, li: [] }, this.storageName = `${this.elements.root.schema.formular}.${this.elements.root.modelId}.${this.elements.in.id}`, this.markedItem = -1, !this.historyInputResultAttr in this.elements.in) throw new Error(`Referenced ELement has no ${this.historyInputResultAttr} attribute;`);
                                this.obtainHistorySource()
                            }
                            obtainHistorySource() { "local" === this.historySource ? (this.history = this.filteredHistory = JSON.parse(localStorage.getItem(this.storageName) || "[]"), this.createHistory()) : "global" === this.historySource && window.addEventListener("message", this.onMessageSourceListener.bind(this, this.createHistory)) }
                            createHistory() { this.createDropdown(), this.setupListeners() }
                            applyFilter() { this.filteredHistory = this.history.filter(e => Boolean(e)).filter(e => e.toLowerCase().startsWith(this.elements.in[this.historyInputResultAttr].toLowerCase())), this.createListElements() }
                            createDropdown() { this.isVisible = !1, this.className = "history hidden", this.elements.ul = document.createElement("ul"), this.createListElements(), this.append(this.elements.ul) }
                            showDropdown() { this.classList.remove("hidden"), this.isVisible = !0 }
                            hideDropdown() {
                                this.classList.add("hidden");
                                let e = document.querySelector("li.marked");
                                e && e.classList.remove("marked"), this.markedItem = -1, this.isVisible = !1
                            }
                            chooseEntry(e) { this.elements.in[this.historyInputResultAttr] = e, this.elements.in.dispatchEvent(new Event("input", { bubbles: !0, cancelable: !0 })), this.hideDropdown() }
                            createListElements(e) {
                                this.elements.li = [], this.elements.ul.innerHTML = "", this.markedItem = this.markedItem > -1 ? e || 0 : -1, this.filteredHistory.forEach((e, t) => {
                                    let s = document.createElement("li");
                                    this.elements.li.push(s), t === this.markedItem && s.classList.add("marked"), s.innerText = e, this.elements.ul.append(s)
                                })
                            }
                            moveMarkedItem(e, t) {-1 !== e && this.elements.li[e].classList.remove("marked"), -1 === e ? this.markedItem = -1 === t ? this.filteredHistory.length - 1 : 0 : 1 === this.filteredHistory.length ? this.markedItem = 0 === e ? -1 : 0 : 0 === e ? this.markedItem = -1 === t ? -1 : e + t : e === this.filteredHistory.length - 1 ? this.markedItem = -1 === t ? e + t : -1 : this.markedItem = e + t, -1 !== this.markedItem && (this.elements.li[this.markedItem].classList.add("marked"), this.elements.ul.scrollTo(0, this.elements.li[this.markedItem].offsetTop)) }
                            onClickListener(e) { "LI" === e.target.nodeName && (e.preventDefault(), this.chooseEntry(e.target.innerText)) }
                            onFocusListener(e) { this.applyFilter(), this.filteredHistory.length > 0 && this.showDropdown() }
                            onBlurListener(e) { this.hideDropdown() }
                            onMouseDownListener(e) { e.preventDefault() }
                            onKeyListener(e) { 13 == e.which && -1 !== this.markedItem && this.chooseEntry(this.filteredHistory[this.markedItem]), 27 == e.which && this.isVisible && (this.hideDropdown(), e.preventDefault()), 38 == e.which && this.filteredHistory.length > 0 && (this.moveMarkedItem(this.markedItem, -1), this.showDropdown(), e.preventDefault()), 40 == e.which && this.filteredHistory.length > 0 && (this.moveMarkedItem(this.markedItem, 1), this.showDropdown(), e.preventDefault()), e.ctrlKey || 46 != e.which || "local" !== this.historySource || -1 !== this.markedItem && (this.removeValue(this.filteredHistory[this.markedItem]), 0 === this.history.length && this.hideDropdown(), e.preventDefault()) }
                            onInputListener(e) { this.applyFilter() }
                            onMessageListener(e) { try { let t = JSON.parse(e.data); "submit-msg" === t.messageType && "local" === this.historySource ? this.recordValue(t.messageData[this.elements.in.id]) : "clear-storage" === t.messageType && "local" === this.historySource && t.messageData[this.elements.in.id] && localStorage.setItem(this.storageName, "[]") } catch (e) {} }
                            onMessageSourceListener(e, t) { let s = JSON.parse(t.data); "history-source-models" === s.messageType && (this.history = this.filteredHistory = removeDuplicates(s.messageData.map(e => e[this.elements.in.id])), e.bind(this)()) }
                            setupListeners() { this.elements.ul.addEventListener("click", this.onClickListener.bind(this)), this.elements.ul.addEventListener("mousedown", this.onMouseDownListener.bind(this)), this.elements.in.addEventListener("focus", this.onFocusListener.bind(this)), this.elements.in.addEventListener("blur", this.onBlurListener.bind(this)), this.elements.in.addEventListener("keydown", this.onKeyListener.bind(this)), this.elements.in.addEventListener("input", this.onInputListener.bind(this)), window.addEventListener("message", this.onMessageListener.bind(this)) }
                            recordValue(e) { if (console.log({ operation: "record", element: this.elements.in.id, value: e }), !Boolean(e)) return; - 1 === this.history.findIndex(t => e.toLowerCase() === t.toLowerCase()) && this.history.push(e), localStorage.setItem(this.storageName, JSON.stringify(this.history)) }
                            removeValue(e) { console.log({ operation: "remove", element: this.elements.in.id, value: e }), this.history = this.history.filter(t => t.toLowerCase() !== e.toLowerCase()), this.filteredHistory = this.history.filter(e => Boolean(e)).filter(e => e.toLowerCase().startsWith(this.elements.in[this.historyInputResultAttr].toLowerCase())), this.markedItem === this.filteredHistory.length && (this.markedItem -= 1), localStorage.setItem(this.storageName, JSON.stringify(this.history)), this.createListElements(this.markedItem) }
                        }
                        customElements.define("history-input-extender", HistoryInputExtender);

                    }, {}],
                    6: [function(require, module, exports) {
                                const { InputField: InputField } = require("./input-field.js");
                                module.exports.InfoFieldSummary = class extends InputField {
                                        constructor() { super(), this.defaultOptions = {...this.defaultOptions, observed_fields: [] } }
                                        countFields() {
                                            let e = {};
                                            return this.options.observed_fields.forEach(t => {
                                                document.querySelectorAll(`#${t}`).forEach(t => {
                                                    let n = t.parentElement.parentElement.getModel();
                                                    n && n.forEach(t => { e[t] = void 0 === e[t] ? 1 : e[t] + 1 })
                                                })
                                            }), e
                                        }
                                        runCounting() {
                                                let e = this.countFields();
                                                this.resultList.innerHTML = `${Object.keys(e).map(t=>`<li>${e[t]} x <span class="span-fett">${t}</span></li>`).join("")}`}applyTemplate(){this.rootElement.insertAdjacentHTML("beforeend",`\n          <div class="form-element">\n              ${this.options.label?`<label for="${this.options.name}">${this.options.label}</label><br>`:""}\n              <ul id=${this.options.name}></ul>\n          </div>\n      `),this.resultList=this.querySelector(`#${this.options.name}`),document.querySelector(".form-root").addEventListener("form-valid",this.runCounting.bind(this)),setTimeout(this.runCounting.bind(this),1e3)}getModel(){}};
