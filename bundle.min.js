...
const{InputFieldObject:InputFieldObject}=require("./input-field-object.js"),{fieldTypeMap:fieldTypeMap}=require("./formular-components.js");var baseUrl="http://10.19.28.94:8000",schemaPath="/schema",modelPath="/model";Object.keys(fieldTypeMap).forEach(e=>{customElements.define(fieldTypeMap[e].tag,fieldTypeMap[e].conName)});class SearchParams{constructor(e){let t=this;e.slice(1).split("&").forEach(function(e){let o=e.split("=")[0],l=e.split("=")[1];t[o]=l})}append(e,t){this[e]=t}get(e){return this[e]}set(e,t){this[e]=t}delete(e){delete this[e]}toString(){return"?"+Object.keys(this).map(e=>`${e}=${this[e]}`).join("&")}}function prepareModel(e,t){let o="",l=(new Date).getTime();try{o=wikiContext.UserName}catch(e){console.log("wikiContext not in scope.")}return e["#parentForm"]=t,e["#changed_user"]=o,e["#changed_time"]=l,JSON.stringify(e)}function uploadNewModel(e,t){let o=prepareModel(e,t);return fetch(`${baseUrl}${modelPath}`,{method:"POST",body:`${o}`,headers:{"Content-Type":"application/json"}}).then(e=>e.json()).then(e=>e["#modelID"])}function uploadExistingModel(e,t){let o=prepareModel(e,t);return fetch(`${baseUrl}${modelPath}/${e["#modelID"]}`,{method:"POST",body:o,headers:{"Content-Type":"application/json"}}).then(e=>e.json()).then(e=>console.log(e))}function loadModelFromDB(e){return fetch(`${baseUrl}${modelPath}/${e}`).then(e=>e.json()).then(e=>(console.log(e),e))}function loadSchemaFromDB(e){return fetch(`${baseUrl}${schemaPath}/${e}`).then(e=>e.json())}function getSchemaId(){return new SearchParams(location.search).get("schema")}class FormCreator extends InputFieldObject{constructor(){super(),this.model={}}connectedCallback(){this.rootElement=document.createElement("form"),this.rootElement.classList.add("form-root"),this.appendChild(this.rootElement),loadSchemaFromDB(getSchemaId()).then(e=>{console.log(e),this.applySchema(e)})}applySchema(e){console.log(e),this.schema=e,this.options.name=e.formular,this.options.label=`${e.formular}`,this.options.subform=e.felder;try{let t=new SearchParams(location.search).get("mid");t?loadModelFromDB(t).then(o=>{this.model=this.convertValue("initialModel",JSON.stringify(o)),this.model["#modelID"]=t,this.options.name=e.formular,this.options.label=`${this.options.label} ${t}`,this.options.initialModel=this.convertValue("initialModel",JSON.stringify(o)),this.rootElement.options=this.options,this.applyTemplate()}):(this.options.initialModel=this.loadFromLocal(),this.rootElement.options=this.options,this.applyTemplate())}catch(e){console.log(this.options.initialModel),console.error(e)}this.uploadModelButton(),this.newFormularButton(),this.newRemoveButton(),this.renderGeneralInfo(e)}renderGeneralInfo(e){this.insertAdjacentHTML("afterbegin",`\n            <div class="formular-title">\n                <h2>${e.formular}</h2>\n            </div>\n        `)}testModelCreation(){let e=document.createElement("button");e.setAttribute("type","button"),e.addEventListener("click",e=>{this.checkValidity()?(this.model={...this.model,...this.getModel()},this.saveFormLocal(this.model["#modelID"],this.model)):console.error("Einige Formular Elemente sind nicht korrekt ausgefüllt.")}),e.innerText="Erzeuge Model",this.appendChild(e)}createNewFormURL(){let e=new URL(location.href),t=new SearchParams(location.search);return t.set("lsid","null"),t.delete("mid"),e.search=t.toString(),e.href}newFormularButton(){let e=document.createElement("button");e.setAttribute("type","button"),e.addEventListener("click",()=>{location.href=this.createNewFormURL()}),e.innerText="Neu Anlegen",this.appendChild(e)}newRemoveButton(){let e=document.createElement("button");e.setAttribute("type","button"),e.addEventListener("click",()=>{this.model["#modelID"]&&confirm(`Are you sure, you want to renove model with id: ${this.model["#modelID"]}?`)&&fetch(`${baseUrl}${modelPath}/${this.model["#modelID"]}`,{method:"DELETE"}).then(e=>e.ok?location.href=this.createNewFormURL():console.error(e.status))}),e.innerText="Remove Model",this.appendChild(e)}uploadModelButton(){let e=document.createElement("button");e.setAttribute("type","button"),e.addEventListener("click",e=>{if(this.checkValidity()){let e=this.getModel();void 0===e?console.log("empty model"):(this.model={...this.model,...e},this.saveFormLocal(this.model["#modelID"],this.model),this.model["#modelID"]?(console.log(this.model),uploadExistingModel(this.model,this.schema.formular).then(()=>alert(`Änderungen an ${this.model["#modelID"]} wurden gespeichert.`))):uploadNewModel(this.model,this.schema.formular).then(e=>(this.model["#modelID"]=e,this.querySelector(`label[for="${this.schema.formular}"]`).innerText=`${this.options.label} ${e}`,alert(`Daten wurden erfolgreich in der Datenbank unter folgender ID abgelegt.\n${e}`))))}else console.log("invalid")}),e.innerText="Speichern",this.appendChild(e)}loadFromLocal(e){return e||(e=null===(e=new SearchParams(location.search).get("lsid"))?"last":e),this.convertValue("initialModel",localStorage.getItem(e))}saveFormLocal(e,t){let o=this.saveValue("model",t);console.group("saving model to localStorage"),e&&(localStorage.setItem(`${e}-${this.schema.formular}`,o),console.log("saved as:",`${e}-${this.schema.formular}`));let l=parseInt(localStorage.getItem("last-x"));Number.isNaN(l)||l>=10?(localStorage.setItem("last-x",0),localStorage.setItem("last-0",o),console.log("saved as:","last-0")):(localStorage.setItem("last-x",l+1),localStorage.setItem(`last-${l+1}`,o),console.log("saved as:",`last-${l+1}`)),localStorage.setItem("last",o),console.log("saved as:","last"),console.groupEnd()}}customElements.define("form-creator",FormCreator);

},{"./formular-components.js":3,"./input-field-object.js":10}],3:[function(require,module,exports){
const{InputFieldText:InputFieldText,InputFieldEnumListText:InputFieldEnumListText,EnumListableMixin:EnumListableMixin,InputFieldEmail:InputFieldEmail,InputFieldTel:InputFieldTel,InputFieldDate:InputFieldDate,InputFieldNumber:InputFieldNumber}=require("./input-field-generic.js"),{InputFieldTextarea:InputFieldTextarea}=require("./input-field-textarea.js"),{InputFieldBoolean:InputFieldBoolean}=require("./input-field-boolean.js"),{InputFieldDropdown:InputFieldDropdown}=require("./input-field-dropdown.js"),{InputFieldRadio:InputFieldRadio}=require("./input-field-radio.js"),{InputFieldLookup:InputFieldLookup}=require("./input-field-lookup.js"),{InputFieldAbhaengig:InputFieldAbhaengig,DependenceMixin:DependenceMixin}=require("./dependent-fields.js"),{InputFieldList:InputFieldList}=require("./input-field-list.js"),{InputFieldObject:InputFieldObject}=require("./input-field-object.js"),{InputFieldChooseList:InputFieldChooseList}=require("./input-field-choose-list.js"),InputFieldDependentEnumTextarea=class extends(EnumListableMixin(DependenceMixin(InputFieldTextarea))){constructor(){super()}},InputFieldEnumTextArea=class extends(EnumListableMixin(InputFieldTextarea)){constructor(){super()}},InputFieldDependentChooseList=class extends(DependenceMixin(InputFieldChooseList)){constructor(){super()}},fieldTypeMap=module.exports.fieldTypeMap={text:{tag:"input-field-text",conName:InputFieldText},enumtext:{tag:"input-field-enumlisttext",conName:InputFieldEnumListText},enumtextarea:{tag:"input-field-enumlisttextarea",conName:InputFieldEnumTextArea},dependentenumtextarea:{tag:"input-field-dependentenumtextarea",conName:InputFieldDependentEnumTextarea},email:{tag:"input-field-email",conName:InputFieldEmail},tel:{tag:"input-field-tel",conName:InputFieldTel},date:{tag:"input-field-date",conName:InputFieldDate},number:{tag:"input-field-number",conName:InputFieldNumber},textarea:{tag:"input-field-textarea",conName:InputFieldTextarea},boolean:{tag:"input-field-boolean",conName:InputFieldBoolean},dropdown:{tag:"input-field-dropdown",conName:InputFieldDropdown},radio:{tag:"input-field-radio",conName:InputFieldRadio},lookup:{tag:"input-field-lookup",conName:InputFieldLookup},dependenttext:{tag:"input-field-abhaengig",conName:InputFieldAbhaengig},list:{tag:"input-field-list",conName:InputFieldList},object:{tag:"input-field-object",conName:InputFieldObject},choose:{tag:"input-field-choose-list",conName:InputFieldChooseList},dependentchoose:{tag:"input-field-dependent-choose-list",conName:InputFieldDependentChooseList}};
